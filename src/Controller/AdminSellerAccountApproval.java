...
public class AdminSellerAccountApproval implements Initializable {
    ...
    @FXML
    private AnchorPane paneRiderOrderHistory;
    @FXML
    private Label lblWelcome;
    @FXML
    private ImageView iconCart, iconHome;
    @FXML
    private TableView<Seller> tableView;
    @FXML
    private TableColumn<Seller, Object> colAccountId, colName, colEmail, colMobileNo, colAccountType;
    @FXML
    private TableColumn<Seller, Seller> colAction;

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        data.setSellers(new ArrayList<>());
        ArrayList<HashMap<String, Object>> as = db.readAll(String.format(
                "SELECT s.address AS sellerAddress,sh.address AS shopAddress,s.status AS sellerStatus,sh.status AS shopStatus,a.*,s.*,sh.* FROM `Account` a, `Seller` s, `Shop` sh WHERE a.type='Seller' AND a.accountID=s.accountID AND s.shopID=sh.shopID AND s.status=0"));
        for (HashMap<String, Object> a : as) {
            Shop shop = new Shop(a.get("shopID"), a.get("shopName"), a.get("address"), a.get("tel"), a.get("startHour"),
                    a.get("endHour"), a.get("status"), a.get("dateCreated"), a.get("deliveryFee"), a.get("imgPath"));
            Seller seller = new Seller(a.get("accountID"), a.get("username"), a.get("password"), a.get("name"),
                    a.get("email"), a.get("mobileNo"), a.get("type"), a.get("regDate"), a.get("sellerID"),
                    a.get("address"), a.get("NRIC"), a.get("licenseNumber"), a.get("bankAcc"), a.get("shopID"),
                    a.get("status"));
            seller.setShop(shop);
