...
public class Shop implements TableDataProcessing {
    ...
    public Shop(Object shopID, Object name, Object address, Object tel, Object startHour, Object endHour, Object status,
            Object dateCreated, Object deliveryFee, Object imgPath) {
    ...
    public void loadFoods() {
        ...
        ArrayList<HashMap<String, Object>> foodMap = db
                .readAll(String.format("SELECT * FROM `Food` WHERE shopID='%s'", shopID));
        for (HashMap<String, Object> f : foodMap) {
            this.foods.add(new Food(f.get("foodID"), f.get("foodName"), f.get("foodDesc"), f.get("imgPath"),
                    f.get("price"), f.get("category"), f.get("shopID")));
        }
        try {
    ...
    public void loadPendingOrders() {
        ...
        // https://stackoverflow.com/questions/19196475/custom-order-by-in-sql-server-like-p-a-l-h
        // (select data order by the custom sequence)
        ArrayList<HashMap<String, Object>> os = db
                .readAll(String.format("SELECT * FROM `Order` WHERE shopID='%s' AND status='Pending'", shopID));
        for (HashMap<String, Object> o : os) {
            this.orders.add(new Order(o.get("orderID"), o.get("status"), o.get("dateCreated"), o.get("timeCreated"),
                    o.get("buyerID"), o.get("riderID"), o.get("shopID"), o.get("paymentID"), o.get("reviewID")));
        }
    }
    ...
    public void loadAcceptedOrders() {
        ...
        // https://stackoverflow.com/questions/19196475/custom-order-by-in-sql-server-like-p-a-l-h
        // (select data order by the custom sequence)
        ArrayList<HashMap<String, Object>> os = db.readAll(String.format(
                "SELECT * FROM `Order` WHERE shopID='%s' AND status<>'Pending' ORDER BY CASE WHEN status='Rider Accepted' THEN 1 WHEN status='Seller Accepted' THEN 2 WHEN status='Seller Ready' THEN 3 WHEN status='Rider Collected' THEN 4 WHEN status='Completed' THEN 5 ELSE 6 END ASC",
                shopID));
        for (HashMap<String, Object> o : os) {
            this.orders.add(new Order(o.get("orderID"), o.get("status"), o.get("dateCreated"), o.get("timeCreated"),
                    o.get("buyerID"), o.get("riderID"), o.get("shopID"), o.get("paymentID"), o.get("reviewID")));
    ...
    public void acceptOrder(String orderID) {
        String accept = "Seller Accepted";
        sql.updateOrderStatus(orderID, accept);
    }

    public void declineOrder(String orderID) {
        String decline = "Seller Declined";
        sql.updateOrderStatus(orderID, decline);
    }

    public void readyOrder(String orderID) {
        String ready = "Seller Ready";
        sql.updateOrderStatus(orderID, ready);
    }

    public void loadReviews() {
        this.reviews = new ArrayList<>();
        ArrayList<HashMap<String, Object>> reviews = db.readAll(String.format(
                "SELECT r.*,a.username FROM `Review` r,`Order` o,`Buyer` b,`Account` a WHERE r.shopID='%s' AND r.orderID=o.orderID AND o.buyerID=b.buyerID AND b.accountID=a.accountID",
                shopID));
        for (HashMap<String, Object> r : reviews) {
            Review review = new Review(r.get("reviewID"), r.get("rating"), r.get("comment"), r.get("dateCreated"),
                    r.get("timeCreated"), r.get("orderID"), r.get("shopID"));
    ...
    public void edit(String name, String address, String tel, LocalTime startHour, LocalTime endHour,
            double deliveryFee, String imgPath) {
        this.name = name;
        this.address = address;
        this.tel = tel;
        this.startHour = startHour;
        ...
        Task<Void> task = new Task<Void>() {
            ...
            public Void call() throws IOException, SQLException {
                db.executeCUD(String.format(
                        "UPDATE `Shop` SET shopName='%s', address='%s', tel='%s', startHour='%s', endHour='%s', deliveryFee=%.2f, imgPath='%s' WHERE shopID='%s'",
                        name, address, tel, startHour, endHour, deliveryFee, imgPath, shopID), gui);
                return null;
            }
        };
    ...
    public HashMap<String, ArrayList<Food>> getFoodObjArrThatMapWithCategory() {
        ...
        ArrayList<HashMap<String, Object>> foodTable = db
